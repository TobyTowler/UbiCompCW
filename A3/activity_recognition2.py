...
def compute_raw_data(dir_name):
    ...
    for session in sessions:
        ...
        accel = np.stack(
            [
                np.interp(timestamps, accel[:, 0], accel[:, 1]),
        ...
        bar = sm.nonparametric.lowess(
            bar[:, 0], timestamps, return_sorted=False
        ).reshape(-1, 1)
        ...
        labels = np.array(bar.shape[0] * [int(activity_indices[session[1]])]).reshape(
            -1, 1
        )
        ...
        if raw_data_features is None:
            ...
            raw_data_features = np.append(
                raw_data_features, np.append(accel, bar, axis=1), axis=0
            )
            raw_data_labels = np.append(raw_data_labels, labels, axis=0)
            interpolated_timestamps = np.append(
                interpolated_timestamps, timestamps, axis=0
            )
...
def plot_raw_data(raw_data_features, raw_data_labels):
    ...
    """
    accel_magnitudes = np.sqrt(
        (raw_data_features[:, 0] ** 2).reshape(-1, 1)
...
def feature_extraction(raw_data_features, raw_data_labels, timestamps):
    ...
    for i in range(0, accel_magnitudes.shape[0] - segment_size, 64):
        ...
        for j in range(len(freqs) - 1):
            log_band_power.append(
                sum(segment_fft_powers[freqs[j] : freqs[j + 1]]).tolist()[0]
            )

        # Slope of barometer data
        # bar_slope = raw_data_features[i+segment_size-1, 3] - raw_data_features[i, 3]
        bar_slope = np.polyfit(
            timestamps[i : i + segment_size],
            raw_data_features[i : i + segment_size, 3],
            1,
        )[0]
        # bar_slope = np.polyfit([x*0.1 for x in range(segment_size)], raw_data_features[i:i+segment_size, 3], 1)[0]

        feature = (
            [accel_mean, accel_var] + equal_band_power + log_band_power + [bar_slope]
        )
        ...
        label = Counter(
            raw_data_labels[i : i + segment_size][:, 0].tolist()
        ).most_common(1)[0][0]
...
if __name__ == "__main__":
    ...
    features, labels = feature_extraction(
        raw_data_features, raw_data_labels, timestamps
    )
    ...
    for dir in dirs:
        ...
        raw_data_features, raw_data_labels, timestamps = compute_raw_data(
            data_path + dir
        )
        features, labels = feature_extraction(
            raw_data_features, raw_data_labels, timestamps
